...
func init() {
	if err := db.CreateCollection(userCollection); err != nil {
...
func StoreUserInDb(u *User, password string) error {
	...
	v, _ := db.GetDataFromCollection(userCollection, userInfo.ID)
	...
	return db.AddDataToCollection(userCollection, userInfo.ID, userData)
}
...
func UpdateData(oldData, newData *User) error {
	...
	if oldData.UserName != newData.UserName {
		rawData, _ := db.GetDataFromCollection(userCollection, GetUserID(newData))
		...
		rawData, _ = db.GetDataFromCollection(userCollection, GetUserID(oldData))
		err := db.DeleteDataFromCollection(userCollection, GetUserID(oldData))
		...
		err = db.AddDataToCollection(userCollection, GetUserID(&u.User), uRaw)
	...
	return db.AddDataToCollection(userCollection, GetUserID(&userInfo.User), rawData)
}

// GetAllUsersInDb returns all users contained in the database.
func GetAllUsersInDb() (data []User, err error) {
	allRawUsers, err := db.GetAllDataFromCollection(userCollection)
...
func ValidateUser(u *User, password string) (validated bool, err error) {
	rawUser, err := db.GetDataFromCollection(userCollection, GetUserID(u))
...
func GetUser(userName string) (userInfo *User, err error) {
	rawUserData, err := db.GetDataFromCollection(userCollection, GetUserIDFromName(userName))
...
func DeleteUser(userName string) error {
	return db.DeleteDataFromCollection(userCollection, GetUserIDFromName(userName))
}
...
func ChangePassword(userName, oldPassword, newPassword string) error {
	...
	return db.AddDataToCollection(userCollection, GetUserID(&u.User), updatedRawUser)
}
